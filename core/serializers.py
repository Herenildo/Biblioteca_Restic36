...
class ListaLivrosCategoriaSerializer(serializers.Serializer):
    categoria=serializers.ReadOnlyField(source='categoria.nome')
    livro=serializers.ReadOnlyField(source='livro.nome')
    class Meta:
        model = Livro
        fields = ['livro','categora']

class ListaLivrosAutorSerializer(serializers.Serializer):
